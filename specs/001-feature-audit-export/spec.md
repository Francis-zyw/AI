...
**Feature Branch**: `feature/001-feature-audit-export`  
**Created**: 2026-04-16  
**Status**: Draft v2  
**Spec Directory**: `specs/001-feature-audit-export/`  
...
- **SC-001**: 审核人员能在 3 秒内加载 1000+ 行 Step3 结果并完成首屏渲染
- **SC-002**: 审核人员能在单次工具会话中完成全部 69 个构件类型的特征审核，无需切换其他工具
- **SC-003**: 导出的 Excel 按构件类型分 sheet，覆盖所有标记为"待补充"的特征条目，零遗漏
- **SC-004**: 导出的 `wiki_patch.json` 能被 `wiki_patch_import.py` 直接消费，无需人工转换
- **SC-005**: 审核进度保存/恢复后，所有标记状态和批注零丢失
- **SC-006**: 工具使用后，Step5→Step3 的知识反馈闭环使下轮匹配率提升（目标：84.6% 缺口率下降至 60% 以下）
- **SC-007**: 构件属性参考面板展示内容与 `component_source_table.json` 数据一致，覆盖 100% 已有属性

## Assumptions

- 审核人员具备基本的构件属性库知识，能判断特征标签与属性的对应关系
- Step3 结果 JSON 格式稳定，`feature_expression_items[]` 结构不会在 Step5 开发期间变更
- `component_source_table.json` 在审核期间不会同步更新（静态参考）
- `wiki_patch_import.py` 已测试通过，接口稳定
- 浏览器环境支持 localStorage（用于审核进度持久化）
- 单次审核会话涉及的数据量不超过 10,000 条特征条目
- 回填 Excel 的格式与导出 Excel 一致（用户不会改变列结构）
