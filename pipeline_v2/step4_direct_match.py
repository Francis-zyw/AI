...
def load_runtime_config(config_path: str | Path | None) -> Dict[str, Any]:
    ...
    return {
        ...
        "knowledge_base_path": resolve_path_from_config(
            parser.get("paths", "knowledge_base", fallback=""),
            resolved_path,
            must_exist=True,
        ),
        ...
        "knowledge_top_k": parse_optional_int(parser.get("knowledge", "top_k", fallback="")),
        "knowledge_max_chars": parse_optional_int(parser.get("knowledge", "max_chars", fallback="")),
    }
...
def resolve_runtime_options(args: Any) -> Dict[str, Any]:
    ...
    step_model_cfg = load_step_model_config("step4", getattr(args, "models_config", None))
    ...
    return {
        ...
        "knowledge_base_path": merge_runtime_value(
            getattr(args, "knowledge_base", None),
            config_values.get("knowledge_base_path"),
            None,
        ),
        ...
        "knowledge_top_k": merge_runtime_value(
            getattr(args, "knowledge_top_k", None),
            config_values.get("knowledge_top_k"),
            DEFAULT_KNOWLEDGE_TOP_K,
        ),
        "knowledge_max_chars": merge_runtime_value(
            getattr(args, "knowledge_max_chars", None),
            config_values.get("knowledge_max_chars"),
        ...
        "openai_api_key": merge_runtime_value(
            getattr(args, "openai_api_key", None),
            provider_env.get("openai_api_key") or config_values.get("openai_api_key"),
            None,
        ),
        "openai_base_url": merge_runtime_value(
            getattr(args, "openai_base_url", None),
            provider_env.get("openai_base_url") or config_values.get("openai_base_url"),
            None,
        ),
    }
...
def build_prompt_batch_payload(
    ...
    return {
        "rows": [{"local_row": row, "source_component": source_summary} for row in local_batch_rows],
    }
...
def build_prompt_text(
    ...
    row_source_context = batch_payload.get("rows", batch_payload)
    knowledge_context = batch_payload.get("knowledge_context", {})
    instructions = [
        "你是 Step4 构件直匹配复核助手。",
        f"固定构件类型: {component_type}",
        "任务: 基于本地直匹配结果、指定构件类型的属性/计算项目，以及知识库检索出的历史证据，复核并修正项目特征表达式与计算项目。",
        "限制:",
        ...
        "8. 知识库证据只能作为补充上下文，不能压过当前清单原文、当前构件属性和当前单位约束。",
        "9. 只有在构件语义、计量单位、规则口径一致时，才允许复用历史模式。",
        "",
        ...
        json.dumps(row_source_context, ensure_ascii=False, indent=2),
    ]
    if knowledge_context:
        instructions.extend(
            [
                "",
                "【KNOWLEDGE_BASE_WIKI】",
                json.dumps(knowledge_context.get("wiki_pages", []), ensure_ascii=False, indent=2),
                "",
                "【KNOWLEDGE_BASE_RETRIEVAL】",
                json.dumps(knowledge_context.get("row_contexts", []), ensure_ascii=False, indent=2),
            ]
        )
    return "\n".join(instructions)
...
def run_step4_direct_match_pipeline(
    *,
    ...
    wiki_context = wiki_retriever.query_for_step4(normalized_component_type)
    ...
    for batch_number, local_batch_rows in enumerate(row_batches, start=1):
        ...
        if wiki_context:
            prompt_batch_payload["knowledge_context"] = {
