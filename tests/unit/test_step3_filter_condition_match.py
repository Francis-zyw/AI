...
class Step3FilterConditionMatchTests(unittest.TestCase):
    ...
    def test_resolve_runtime_options_prefers_runtime_models_ini_for_step3(self) -> None:
        with patch(
            "pipeline_v2.step3_engine.api.load_step_model_config",
            return_value={
                "model": "openai-codex/gpt-5.4",
                "reasoning_effort": "high",
                "api_key_env": "STEP3_API_KEY",
                "base_url_env": "STEP3_BASE_URL",
            },
        ), patch(
            "pipeline_v2.step3_engine.api.resolve_provider_env",
            return_value={
                "openai_api_key": "step3-key",
        ...
        self.assertEqual(options["openai_api_key"], "step3-key")
        self.assertEqual(options["openai_base_url"], "https://example.invalid/v1")

    def test_resolve_runtime_options_supports_codex_from_step3_runtime_config(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            config_path = root / "runtime_config.ini"
            table_regions_path = root / "table_regions.json"
            components_path = root / "components.json"
            synonym_path = root / "synonym_library.json"
            output_dir = root / "step3_output"

            table_regions_path.write_text("[]", encoding="utf-8")
            ...
            config_path.write_text(
                "\n".join(
                    [
                        ...
                        f"step1_table_regions = {table_regions_path}",
                        f"components = {components_path}",
                        f"synonym_library = {synonym_path}",
                        f"output = {output_dir}",
                        "",
                        "[model]",
                        "model = gpt-5.4",
            ...
            with patch("pipeline_v2.step3_engine.api.load_step_model_config", return_value={}):
                options = resolve_runtime_options(
                    Namespace(
                        config=str(config_path),
                        step1_table_regions=None,
    ...
    def test_apply_runtime_environment_codex_clears_openai_env(self) -> None:
        with patch.dict(
            os.environ,
            {
                "OPENAI_API_KEY": "existing-openai-key",
