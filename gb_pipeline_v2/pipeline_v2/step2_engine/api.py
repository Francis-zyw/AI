...
def run_openai_startup_check(
    *,
    ...
    if not api_key:
        raise RuntimeError(
            "未检测到 OPENAI_API_KEY。请先在终端执行 export OPENAI_API_KEY=\"your_api_key_here\" 后再运行。"
        )
...
def build_synonym_library(mappings: Sequence[Dict[str, Any]], result_meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for item in mappings:
        canonical_name = normalize_optional_text(item.get("selected_standard_name", ""))
        ...
        group["aliases"] = deduplicate_preserve_order(
            list(group["aliases"])
            + [canonical_name]
            + item.get("standard_aliases", [])
            + [item.get("source_component_name", "")]
            + item.get("source_aliases", [])
        )
        ...
        group["evidence_paths"] = deduplicate_preserve_order(
            list(group["evidence_paths"]) + item.get("evidence_paths", [])
        )

        note = item.get("reasoning", "").strip()
        if note:
            group["notes"] = deduplicate_preserve_order(list(group["notes"]) + [note])

    synonym_library = sorted(grouped.values(), key=lambda item: item["canonical_name"])
    ...
    return {
        ...
        "unmatched_components": sorted(unmatched),
    }
